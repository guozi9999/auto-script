...
3. 点击列表中的积木可以编辑参数，例如坐标、文本、图片路径、颜色值。
4. 长按积木可以删除。
5. 拖动积木可以调整执行顺序。
...
条件分支由「如果...」「否则」「结束条件」组成。每个「如果」必须有一个对应的「结束条件」；「否则」是可选的，并且必须放在「如果」和「结束条件」之间。

常用条件积木：

- 「如果找到文本」：当 `findText("文本")` 为 true 时执行分支内容。
- 「如果颜色匹配」：当 `colorMatch(x, y, "#RRGGBB")` 为 true 时执行分支内容。
- 「如果找到图片」：当 `findImage("图片路径")` 找到模板图时执行分支内容。
- 「如果自定义条件」：直接填写 JavaScript 条件表达式，例如 `count > 3`。
- 「否则」：前面的条件不成立时执行。
- 「结束条件」：结束当前条件分支。
...
写入文件（覆盖）。
```javascript
writeFile("/sdcard/data.txt", "Hello World");
```
